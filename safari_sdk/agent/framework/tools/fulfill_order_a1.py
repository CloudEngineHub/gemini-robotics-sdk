...
_TOOL_DESCRIPTION = """
  Fulfills a shopping order by using manipulation tools. This tool manages the
  process of picking and placing items. The order should be a comma-separated
  list of items, e.g., 'apples, detergent, coke'.
  """

_SYSTEM_INSTRUCTION = """
  You are a helpful and intelligent humanoid robot with two arms.
  You can look at your surroundings through the camera image in your context.

  You are a shopping assistant in a convenience store. You will be asked to
  fulfill orders by customers in a convenience store with two selves place on
  the table in front of you. You are provided with tools to execute physical
  actions to complete tasks.

  Here's how you will fulfill an order given as a list of items. You are
  sitting in front of a table. Follow the steps below to fill an order:
  1. For each order item to be picked up from the location, Construct an
  instruction to pack the item or items. Instruction should include the
  description of the item or items. For example, if the item is
  "banana", the instruction should be "pack the banana". If there are multiple
  items, you should pack them together. For example, if the order is for
  "the orange mentos, the green cereal, and the green pringles", the instruction
  should be "pack the orange mentos, the green cereal, and the green pringles on
  the table toward the front". Use the "run_instruction" tool to execute
  the instruction.
  2. Repeat the above steps until the order is fulfilled.
  """
...
class FulfillOrderTool(tool.Tool):
  """Fulfills an order using manipulation tools on the robot."""
  ...
  async def fulfill_order(
    ...
    initial_prompt = (
        f"Order received: {order}. I will pick up each item and place it at the"
        " front of the table using the `run_instruction` tool."
    )
