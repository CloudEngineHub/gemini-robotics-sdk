...
class LeRobotEpisodicLogger:
  ...
  def __init__(
    ...
    self._proprioceptive_observation_keys = (
        proprioceptive_observation_keys or []
    )
    ...
    if features:
      (
          ...
          self._action_spec,
          parsed_proprio_keys,
      ) = self._parse_features_to_specs(features)
      if not self._proprioceptive_observation_keys:
        self._proprioceptive_observation_keys = parsed_proprio_keys

      # Ensures that timestamps are provided in the dataset or generated by the
      # logger, but not both.
      has_timestamp = _TIMESTAMP_KEY in self._timestep_spec.observation  # pyrefly: ignore[not-iterable]
      ...
      new_observation_spec = dict(self._timestep_spec.observation)  # pyrefly: ignore[no-matching-overload]
  ...
  def _parse_features_to_specs(
      self,
      features: dict,
  ) -> tuple[gdmr_types.TimeStepSpec, specs.BoundedArray, list[str]]:
    ...
    for key, feature_info in features.items():
      ...
      if (
          key.startswith(_LEROBOT_OBSERVATION_KEY_PREFIX)
          or key == _LEROBOT_ACTION_KEY
      ):
        if _SHAPE_KEY not in feature_info or not feature_info[_SHAPE_KEY]:
          raise ValueError(
              f"Feature '{key}' must specify a non-empty '{_SHAPE_KEY}' list"
              " in dataset features."
          )
      ...
      if key == _LEROBOT_ACTION_KEY:
        ...
        if obs_key in self._image_observation_keys:
          ...
          non_image_obs_keys.append(obs_key)
    ...
    return timestep_spec, action_spec, non_image_obs_keys
...
def convert_lerobot_data_to_mcap(
  ...
  # Validate that shape exists and is not empty for observation and action
  # features.
  if hasattr(dataset, "features"):
    for key, feature_info in dataset.features.items():
      if (
        ...
        if _SHAPE_KEY not in feature_info or not feature_info[_SHAPE_KEY]:
          raise ValueError(
              f"Feature '{key}' must specify a non-empty '{_SHAPE_KEY}' list in"
              " dataset.features."
          )
